 ...
 * and then sort them in order to account for the cells correctly.
 *
 * @opensearch.internal
 */
abstract class GeoShapeCellValues extends AbstractSortingNumericDocValues {
    ...
    public boolean advanceExact(int docId) throws IOException {
        if (geoShapeValue.advanceExact(docId)) {
            // First we need a way to return that values we are going to store in this. so let's create a function
            // for that, which every implementation can implement.
            // sort them
